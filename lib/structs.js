...
const KeyPair = require('./crypto/wallet');
...
class Tx {
    ...
    addApprove(token, amount) {
        if (amount !== "unlimited" && typeof amount !== 'number') {
            throw "approve amount should be number or unlimited";
        }

        let fixedAmount = "unlimited"
        ...
        if(amount !== "unlimited") {
            ...
            fixedAmount = amount.toFixed(Math.max(0, (m[1] || '').length - m[2]));
        }
